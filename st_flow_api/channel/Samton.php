        ...
        foreach ($data as $key => &$value){
            ...
            if( $result['error_code'] == 200)
            {
                ...
                if(  $jsondata['respCode'] == '0000' ) {
                    ...
                    $value['back_content'] .= '->尚通科技[全国漫游]提交成功:'.date('Y-m-d H:i:s');
                    $value['channel_order_code'] = $jsondata['orderID'];

                }else{

                    $value['order_status'] +=3; 
                    $value['back_content'] .= '->尚通科技[全国漫游]提交失败:'.$jsondata['respMsg'].date('Y-m-d H:i:s');
    ...
    public function QueryResult() {
        ...
        foreach ($data as $key => &$value) {
            ...
            if( $result['error_code'] == 200)
            {
                ...
                $value['back_content'] .= '->尚通科技[全国漫游]充值成功(查询) '.date("Y-m-d H:i:s");

                }elseif( $jsondata['respCode']  === '0003' ){

                    $value['order_status'] += 2; //目标是 1->2  4->5
                    $value['back_content'] .= '->尚通科技[全国漫游]充值失败(查询) '.date("Y-m-d H:i:s");
