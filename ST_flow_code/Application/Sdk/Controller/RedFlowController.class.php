...
class RedFlowController extends Controller {
	...
	public function index()
    {
		...
		$user_id = I('user_id' );
        $red_order_code = (int)$red_order_code;
		$this->showinfo($red_order_code,$user_type,$user_id);
    }
	...
	public function showinfo($red_order_code,$user_type,$user_id) 
    {
		//版本号更新css js更新
		...
		// }
		// $red_order_id = $red_order["red_order_id"];
		// $map["red_order_id"] = $red_order_id;
		// $result = M("red_record")->where($map)->find();
		
		
		
		//获取微信信息
		$this->set_shareuserinfo($user_type,$user_id,$red_order_code);
		
        //获取流量红包信息
        $order=M("red_order")->where(array("red_order_code"=>$red_order_code,"pay_status"=>2))->find();

        //购买者的openid
        $buyopenid = $order['wx_openid'];
		...
		$submiturl = "https://api.weixin.qq.com/cgi-bin/user/info?access_token=".$accesstoken."&openid=" .$buyopenid."&lang=zh_CN";
        ...
        if(empty($headimgurl))
        {
            $headimgurl = 'http://'.$_SERVER['HTTP_HOST']."/Application/Sdk/View/RedFlow/images/flow_normal.png";
        }
        else
        {
        ...
        $this->assign("wx_openid",$buyopenid);
        //将购买者信息投影在界面上
		...
		if ($order['out_packages'] == null) {
			...
			$out_num = count($out_packages);
			//领取的数量
		}
		$all_num = count($packages);
		//购买的数量
		$result = array();
		foreach ($packages as $p) {
			...
			if (in_array($p, $out_packages)) {
				...
				if ($product['operator_id'] == 1) {
					$or['product_name'] = "中国移动" . $product["product_name"];
				} elseif ($product['operator_id'] == 2) {
					$order['product_name'] = "中国联通" . $product["product_name"];
				} else {
					$order['product_name'] = "中国电信" . $product["product_name"];
				}
				...
				if ($product['operator_id'] == 1) {
					...
					$or['product_name'] = "中国联通" . $product["product_name"];
				} else {
					$or['product_name'] = "中国电信" . $product["product_name"];
				}
				$or['info'] = "未有人领取";
 ...
 //获取领取记录
   ...
   //纪录录入红包领取记录
	 ...
	 public function set_shareuserinfo($user_type,$user_id,$red_order_code){
	...
	// 	$Openid= $obj['openid'];
        ...
        $Link = gethostwithhttp()."/index.php/Sdk/RedFlow/aindex?".$data;

        //$Link=gethostwithhttp()."/index.php/Sdk/RedFlow/index/user_type/".$user_type."/user_id/".$user_id;
