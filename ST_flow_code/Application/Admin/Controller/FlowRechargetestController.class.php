 ...
 * 流量充值
 */
class FlowRechargetestController extends CommonController {
    ...
    public function index(){
		...
		//$wherecll['status']= 1;
		//$channel_list = M('Channel')->order('channel_id asc')->select();
	 ...
	 * 根据运营商ID和省份ID获取通道
	 */
	public function get_op_channel() {
		if(IS_POST) {
			$data = array();
			$post = I('post.');
			$wherec['operator_id'] = $post['operator_id'];
		//	$wherec['province_id'] = $post['province_id'];
			//$wherec['status']      = 1;
			$channel_list = M('Channel')->where($wherec)->order('channel_id asc')->select();
    ...
    public function commit_flow_recharge() {
    	if(IS_POST) {
    		...
    		$take_effect_time = 1; //生效时间  1立即 2下月
			...
			if(empty($channel_id)) {
				$this->ajaxReturn(array('status' => 'error', 'msg' => '请选择通道 ！'.$channel_id));
			}
    		...
    		if(!empty($pid)) {
    			$cond = array(
						...
						'cp.channel_id'    => array('eq', $channel_id),
    			);
    			...
    			$api_info = D('SysUser')->get_sys_api_by_id($cond1);
    				...
    				$account_info = D('SysUser')->get_account_info(2, 2, 0);
					/*
    				...
    				/*foreach ($arr_mobile_info as $k2 => $v2) {
    					$post_data = array(
								...
								'channel_test_key'		=> 'qwewqeqwewqdXWQDWDD##!@#'
    					);
    					...
    					$rt = https_request('http://121.40.51.6:8182/Submittest.php',$post_data);
    					write_debug_log(array(__METHOD__,'curl充值返回结果==',$rt));
    					if(false !== $rt) {
    						$arr_rt = json_decode($rt, true);
    						if('0000' == $arr_rt['respCode']) {
								$this->ajaxReturn(array('status' => 'success', 'info' => '流量充值已提交，请等待结果！', 'msg'=>'流量充值已提交，请等待结果！'));
    						}elseif($arr_rt['respCode']=='3005'){

								$this->ajaxReturn(array('status' => 'error', 'msg' => '选择测试通道流量包不存在！'));
							}else{

								$this->ajaxReturn(array('status' => 'error', 'msg' => '其他错误！'));
							}
    					} else {
					...
					foreach ($arr_mobile_info as $k2 => $v2) {
						...
						$pre_str = "{$api_key}account={$post_data['account']}&action=Charge&phone={$post_data['phone']}&range={$post_data['range']}&size={$post_data['size']}&timeStamp={$post_data['timeStamp']}{$api_key}";
						$post_data['sign'] = md5($pre_str);
						$rt = https_request(C('COMMIT_URL_TEST'),$post_data);
						//write_debug_log(array(__METHOD__,'curl充值返回结果==',$rt));
						if(false !== $rt) {
							$arr_rt = json_decode($rt, true);
							if('0000' != $arr_rt['respCode']) {
								...
								$count_fail += 1;
								write_error_log(array(__METHOD__.'：'.__LINE__, '订单提交失败，原因：'.$arr_rt['respMsg'].'，编码：'.$arr_rt['respCode'].'，提交地址：'.C('COMMIT_URL_TEST'), $post_data));
							} else {
								$count_succ += 1;
							}
						} else {
