 ...
 * 产品折扣记录类
 */
class ProductDiscountRecordController extends CommonController{
    public function index(){
       ...
       //$where['dr.top_proxy_id']=D('SysUser')->self_proxy_id();
        ...
        $count= M('discount_product_record as dr')
        ...
        $show       = $Page->show();
        $list= M('discount_product_record as dr')
            ->join("left join ".C('DB_PREFIX')."enterprise e on e.enterprise_id = dr.enterprise_id and dr.user_type=2")
            ->join("left join ".C('DB_PREFIX')."proxy p on p.proxy_id = dr.proxy_id and dr.user_type=1")
            //->join("left join ".C('DB_PREFIX')."proxy tp on tp.proxy_id = dr.top_proxy_id")
            ->join("left join ".C('DB_PREFIX')."product pr on dr.size = pr.size and dr.operator_id=pr.operator_id")
            ->where($where)
            ->limit($Page->firstRow.','.$Page->listRows)
            ->order("dr.create_date desc")
            ->field("dr.*,e.enterprise_name,e.enterprise_code,p.proxy_name,p.proxy_code,pr.product_name")
            ->group("dr.record_id,pr.product_name")
            ->select();
        //var_dump($list);
    ...
    public function export_excel(){
        ...
        foreach($list as $v){
            ...
            $cash['product_name']=$v['product_name'];
        ...
        $headArr=array("用户编号","用户名称","用户类型","运营商","地区","流量包名称","操作前折扣数","操作后折扣数","操作人","操作时间");
        $name="产品折扣变动记录";
    ...
    public function index_record(){
        ...
        $list= M('discount_product_record as dr')
            ...
            ->limit($Page->firstRow.','.$Page->listRows)
            ->order("dr.create_date desc")
            ->field("dr.*,e.enterprise_name,e.enterprise_code,pr.product_name")
            ->group("dr.record_id,pr.product_name")
            ->select();
        //var_dump($list);
        $operator = D("ChannelProduct")->operatorall();//读取运营商
    ...
    public function export_excel_record(){
        ...
        $headArr=array("用户编号","用户名称","运营商","地区","流量包名称","操作前折扣数","操作后折扣数","操作人","操作时间");
        $name="产品折扣变动记录";
        ExportEexcel($name,$headArr,$data);
    }
